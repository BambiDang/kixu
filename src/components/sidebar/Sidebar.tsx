...
export default function Sidebar({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      setAdminIds(new Set(memberships.filter((m) => m.role === 'admin').map((m) => m.community_id)))
    ...
    <div className="mx-2">
      ...
      {isActive && isAdmin && (
        <Link
          ...
          onClick={onNavigate}
          className="flex items-center gap-1.5 px-2 py-1 ml-8 rounded text-xs text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition-colors"
        >
